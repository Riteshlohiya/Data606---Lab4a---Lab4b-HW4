...
#Set my working directory as C:\Users\Arnav\Documents\R\win-library\3.4\DATA606\labs\Lab4a\more #
...
Answer: The distribution is unimodal and right skewed.
...
The distribution is not perfectly normal. It is right skewed. Also its unimodal.
...
Answer: With every sample the sample mean varies.  The sample mean is close to each other(specially samp1, samp2 and samp3).
...
```{r}
sample_means50_1 <- rep(NA, 50000)

for(i in 1:50000){
   samp <- sample(area, 50)
   sample_means50_1[i] <- mean(samp)
   }

hist(sample_means50_1)
```

```{r}
length(sample_means50_1)
mean(sample_means50_1)
```

There are 5000 elements in sample_means50. The sampling distribution looks like a very good normal distribution, with mean mean very close to population mean. For 50,000 samples, the sampling distribution looks similar, and the mean would also be very close.
...
There are 100 elements in sample_means_small, representing the mean from each 100 samples
...
Answer: When the sample size is large  the center is closer to the mean of the population and and the spread gets narrower.
...
Answer: The one with larger sample size has the smaller spread. In this case the second (sample_means150) has smaller spread. It is preferable to have a distribution with a smaller spread when trying to make estimates.
